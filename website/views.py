...
from .helper_funcs import save_a_quote, sent_normalize, test_language, compare_texts, search_es
    ...
    if request.method == "POST":
        ...
        if not text:
            ...
            if not not (set(text) - allowed_chars):
                ...
                flash("Can't handle some of those symbols: "+str(tmp), category='error')
            elif not test_language(text):
                flash("the spacy tells me you just posted cringe. a possibly ungrammatical sentence", category='error')
            else:
...
@login_required
def like(quote_id):
    ''' handle like functionality, done in static js file; backended here to store likes in user info'''
