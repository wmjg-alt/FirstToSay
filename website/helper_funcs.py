...
# western bias galore
accept_langs = ['en', 'fr', 'es', 'de', 'nl', 'da',
                'nl',  'el', 'it', 'pt', 'ja', 'zh']
...
def test_language(s: str):
    ''' spacy's detect_language is used to decide on how realistic
        a user_input is. Returns True when spacy confidently
        detects one of our accept_langs languages
    '''
    # decided on a spacy threshhold of 0.5
    # it detects gibberish as certain langs
    doc = nlp(s)
    ...
    if len(database_texts) > 0:
        for Q in database_texts:
            ...
            if text_ratio >= 0.95 or matched:
                matched = True
            if text_ratio >= 0.7:
                # followers are for logging authors SIMILAR
...
def write_fail(idx, sent, author, message):
    ''' output the message that failed 
        to be input into the db into a fails log file'''
...
def quote_to_db(text: str, target_user: User, db):
    ''' taking text and a user, add a Quote to Quotes,
        then commit that to db
    '''
...
def ingest_csv_to_dfs(list_of_files: list, db):
    ...
    df_users.drop_duplicates(subset=['email'],
                             keep='first',
                             inplace=True,
                             ignore_index=True)
    ...
    c_c = [col for col in df_qts.columns if col not in ['author', 'quote']]
    df_qts = df_qts.drop(columns=c_c)
...
def pre_fill_db(db, es, index_name: str):
    ''' fill the database with author users and quotes
        from 1/2 million quotes in data/ ingest to db,
        then bulk process to es while logging failures in fail_file
    '''
    ...
    ingest_csv_to_dfs(['data/quotes.csv',
                       'data/quotes_funny.csv',
                       'data/Popular_Quotes.csv',
                       'data/quotes2.csv'], db)
